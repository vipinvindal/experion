...
- (void)layoutSubviews {
	...
	// Position elements
	CGFloat yPos = roundf(((bounds.size.height - totalSize.height-50) / 2)) + margin + yOffset;
...
- (void)drawRect:(CGRect)rect {
	...
	CGRect boxRect = CGRectMake(roundf((allRect.size.width - size.width) / 2) + self.xOffset,
								roundf((allRect.size.height - size.height-50) / 2) + self.yOffset, size.width, size.height);
	float radius = 10.0f;
