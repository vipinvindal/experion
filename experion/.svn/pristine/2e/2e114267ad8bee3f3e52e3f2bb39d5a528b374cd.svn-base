		...
		<tr class = "trBody" align = "center">
			<td>
				<table class="bodyHeader">
					...
					<s:form method="post"
						action="./addLocationMap.html?projectId=${projectDetail.id}"
						modelAttribute="locationMap" enctype="multipart/form-data">
						...
						<tr>
							<td colspan="2" class = "bodyTableTrHeaderFirst">Select Image to upload </td>
...
<tr class = "bodyTableTrHeaderFirst">
								...
								<td colspan="100%">Uploaded Location Maps</td>
						...
						<tr class = "bodyTableTrHeader">
							<c:forEach var="imageType" items="${imageTypeList}">
								<th colspan="0">${imageType.displayName}</th>
							</c:forEach>
							<th>Delete</th>
						...
						<c:forEach varStatus="locationMapMapCount" items="${dataList}">
							<c:set var="locationMapInfoList"
								value="${dataList[locationMapMapCount.index]}" scope="page" />
							<c:set var="commonName"
								value="${commanNameList[locationMapMapCount.index]}" scope="page" />

							<tr class="bodyTableTrNonHeader">

								<c:forEach varStatus="loopCount" items="${locationMapInfoList}">
									<c:set var="locationMapInfo"
										value="${locationMapInfoList[loopCount.index]}" scope="page" />
									<c:set var="imageTypeId"
										value="${imageTypeList[loopCount.index]}" scope="page" />
									<c:choose>

										<c:when test="${not empty locationMapInfo}">
											<td><img
												src="<c:url value='/viewImage.html?imagePath=${locationMapInfo.imagePath}'/>"
												border="1" width="80" height="80"> <s:form
													method="post"
													action="./deleteSingleLocationMap.html?locationMapId=${locationMapInfo.id}&projectId=${projectDetail.id}">
													<input type="submit" value="Delete Image" />
												</s:form></td>
										</c:when>
										<c:otherwise>
											<td><s:form method="post"
													action="./updateLocationMap.html?locationMapCommon=${commonName}&projectId=${projectDetail.id}"
								...
								<td><s:form method="post"
										action="./deleteLocationMap.html?locationMapCommon=${commonName}&projectId=${projectDetail.id}">
